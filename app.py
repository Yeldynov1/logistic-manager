...
st.set_page_config(page_title="LogisticManager v5.9 (Turbo)", page_icon="🚀", layout="wide")
            ...
            for item in r.json()['data']:
                ...
                if ttn:
                    results[ttn] = {
                        ...
                        "Phone": item.get('RecipientPhone', '') # Іноді тут пусто, тоді беремо з наступного кроку
                    }

    return results

# --- ЗВИЧАЙНИЙ ЗАПИТ НП (ДЛЯ ОДИНИЧНИХ ВИПАДКІВ) ---
def get_np_status_full(ttn):
    # Ця функція залишається для сумісності, але основна робота буде в Bulk
    res = get_np_statuses_bulk([ttn])
    if ttn in res:
        return res[ttn]["Status"], res[ttn]["Phone"], "", res[ttn]["Cost"]
    return "", "", "", 0.0
...
def process_status_updates(show_ui=True):
    ...
    # 1. ЗБИРАЄМО ТТН НОВОЇ ПОШТИ ДЛЯ ГРУПОВОГО ЗАПИТУ (TURBO MODE)
    np_ttns = []
    for i, row in df.iterrows():
    ...
    for i, row in df.iterrows():
        ...
        if not any(x in current for x in ['отримано', 'вручено', 'відмова', 'повернення']):
            ...
            if svc == "НП" and ttn in np_cache:
                ...
                s = info['Status']
                cost = info['Cost']
                # Телефони оновлюємо тільки якщо в нас пусто
                if info['Phone'] and len(str(row['Телефон'])) < 10:
                    df.at[i, 'Телефон'] = info['Phone']
                    
            # Якщо УП/Meest -> Робимо звичайний запит (повільно, але надійно)
            elif svc == "УП": 
        ...
        # SMS Logic (Генеруємо текст, якщо прибуло)
...
st.title("📦 LogisticManager (Turbo)")
load_data()
        ...
        with st.form("add"):
            ...
            if st.form_submit_button("Додати") and ttn_in:
                for t in ttn_in.replace(",", " ").split():
                    ...
                    if len(tc) == 12 and tc.isdigit(): tc = "0" + tc # Fix нуля
