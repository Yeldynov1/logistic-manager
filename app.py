...
st.set_page_config(page_title="LogisticManager v5.5 (UP Fix)", page_icon="🚛", layout="wide")
    ...
    if config.UP_BEARER_TOKEN and len(config.UP_BEARER_TOKEN) > 10 and config.UP_USER_TOKEN:
        try:
            ...
            if r.status_code == 200:
                data = r.json()
                # Шукаємо статус в різних полях (структура УП може змінюватись)
                status_raw = data.get('lifecycle', {}).get('status')
                last_event = data.get('lifecycle', {}).get('eventName')
                
                # Повертаємо статус, якщо знайшли
                ...
                return "Не знайдено (API)", None, 0.0
        except Exception as e:
            # st.error(f"UP Business API Error: {e}") # Для дебагу
            pass
    ...
    try:
        ...
        if r and r.status_code == 200:
            ...
            if data and isinstance(data, list):
             ...
             st.toast(f"УП Помилка: {r.status_code} для {barcode}", icon="⚠️")
    except: pass
    ...
    for i, row in work_df.iterrows():
        ...
        if not any(x in current for x in ['отримано', 'вручено', 'відмова', 'повернення']):
            ...
            if svc == "НП":
                ...
                # --- ВИКОРИСТОВУЄМО НОВУ ФУНКЦІЮ SMART ---
