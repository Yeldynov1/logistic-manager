...
st.set_page_config(page_title="LogisticManager v6.4 (Final Clean)", page_icon="🚛", layout="wide")
    ...
    for i, row in df.iterrows():
        ...
        if (len(msg_val) <= 5 or msg_val.lower() == 'nan') and not is_sent:
            if any(x in current_status for x in ['отримано', 'доставлено', 'вручено', 'delivered', 'відділенні']):
                ...
                if link and len(link) > 5 and link.lower() != 'nan': txt_msg += f"Переглянути чек: {link}\n"
                txt_msg += "Щиро дякуємо за покупку!"
                df.at[i, 'Повідомлення'] = txt_msg
                if len(str(row['Телефон'])) > 5: df.at[i, 'Статус СМС'] = 'Не отправлено'
    return df
...
def restore_leading_zero(val):
    # Видаляємо всі можливі лапки та пробіли
    s = str(val).replace("'", "").replace('"', "").strip()
    # Якщо це Укрпошта (12 цифр), додаємо 0
...
def get_np_statuses_bulk(ttn_list):
    ...
    for chunk in chunks:
        ...
        try:
            r = utils.make_request("POST", "https://api.novaposhta.ua/v2.0/json/", json={"apiKey": config.API_KEY_NP, "modelName": "TrackingDocument", "calledMethod": "getStatusDocuments", "methodProperties": {"Documents": documents}})
            if r and r.json()['success']:
                for item in r.json()['data']:
                    ttn = item.get('Number')
                    if ttn: results[ttn] = {"Status": item.get('Status', ''), "Cost": float(item.get('AnnouncedPrice') or 0), "Phone": item.get('RecipientPhone', '')}
        except: pass
    return results
...
def load_data():
    if 'df' not in st.session_state:
        ...
        # --- CLEAN FIX: Чистимо ТТН при завантаженні ---
...
def save_manual(df_to_save):
    try:
        sheet = get_google_sheet(); 
        if sheet: 
            # --- CLEAN FIX: Більше не додаємо апострофи (це і було проблемою) ---
            # Просто зберігаємо таблицю, але gsheets може з'їсти 0.
            # Однак restore_leading_zero це поправить при наступному читанні.
...
def process_status_updates(show_ui=True):
    ...
    for i, row in df.iterrows():
        ...
        ttn = utils.clean_ttn(str(df.at[i, 'ТТН']))
        if len(ttn) == 12 and ttn.isdigit(): ttn = "0" + ttn
    ...
    if pen.empty: st.success("Empty!")
        ...
        for i, r in pen.iterrows():
            with st.container(border=True):
                c1, c2 = st.columns([1, 2]); c1.markdown(f"**{r['Служба']}** `{r['ТТН']}`\n{r['Статус']}"); c1.markdown(f"📞 **{r['Телефон']}**")
                
                with c2:
                    current_link = str(r['Чек'])
                    if len(current_link) < 5:
                        new_link = st.text_input("➕ Чек", key=f"lnk_{i}", placeholder="https://check...")
                        if new_link:
                            st.session_state.df.at[i, 'Чек'] = new_link
                            new_msg = f"Доброго дня!\nВаше замовлення отримано.\nПереглянути чек: {new_link}\nЩиро дякуємо за покупку!"
                            st.session_state.df.at[i, 'Повідомлення'] = new_msg
                            
                            # CLEAN FIX: Видаляємо ключ з сесії, щоб текст оновився
                            if f"t{i}" in st.session_state: del st.session_state[f"t{i}"]
                            
                            save_manual(st.session_state.df); st.rerun()
                    
                    # Використовуємо ключ "t{i}" для текстового поля
                    st.text_area("", r['Повідомлення'], height=70, key=f"t{i}", label_visibility="collapsed")
                
                render_smart_buttons(r['Телефон'], r['Повідомлення'])
