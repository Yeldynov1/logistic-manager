...
with st.sidebar:
    ...
    with st.expander("� Відновити чеки з файлу по ТТН", expanded=False):
        st.info("💡 Завантажте файл зі старої версії (ТТН + Чек). Система зіставить по ТТН і заповнить пусті чеки.")
        checks_file = st.file_uploader("Оберіть файл (XLSX/CSV) - колонка 1: ТТН, колонка 2: Чек", type=['xlsx', 'csv'], key="checks_uploader")
        if checks_file:
            try:
                checks_df = None
                
                if checks_file.name.endswith('.csv'):
                    ...
                    for enc in encodings:
                        for sep in separators:
                            try:
                                df_test = pd.read_csv(checks_file, dtype=str, encoding=enc, sep=sep)
                    ...
                    checks_df = pd.read_excel(checks_file, dtype=str)
                
                if checks_df is not None:
                    checks_df.columns = checks_df.columns.str.strip()
                    ttn_col = checks_df.columns[0]
                    check_col = checks_df.columns[1]
                    
                    st.info(f"📋 Прочитано {len(checks_df)} рядків: **{ttn_col}** → ТТН, **{check_col}** → Чек")
                    ...
                    if st.button("🔗 Зіставити чеки по ТТН", key="btn_restore_checks"):
                        ...
                        for _, row in checks_df.iterrows():
                            ...
                            check_url = str(row[check_col]).strip()
                            
                            if not ttn_raw or ttn_raw.lower() == 'nan':
                                continue
                            if not check_url or check_url.lower() == 'nan':
                                continue
                            ...
                            checks_dict[ttn_clean] = check_url
                        
                        # Зіставляємо з поточною таблицею
                        updated = 0
                        skipped_empty_ttn = 0
                        ...
                        for i, row in checks_restore_df.iterrows():
                            ...
                            # Пропускаємо якщо чек вже заповнений
                            if current_check and current_check.lower() != 'nan' and len(current_check) > 5:
                                skipped_full_check += 1
                                continue
                        ...
                        st.success(f"""
                        ✅ **Результат:**
                        - 🔗 Заповнено чеків: **{updated}**
                        - ⏭️ Пропущено (вже мають чек): **{skipped_full_check}**
                        - ❌ ТТН не знайдено у файлі: **{not_found}**
                        - ⚠️ Пусті ТТН: **{skipped_empty_ttn}**
                        """)
                        if updated > 0:
