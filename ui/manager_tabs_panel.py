    ...
    with st.expander("👤 Доступ менеджера", expanded=False):
        ...
        with c1:
            if st.button("Зберегти", key="mgr_tabs_save", use_container_width=True):
                if save_manager_tab_visibility(draft):
                    st.toast("Доступ менеджера збережено", icon="✅")
                    st.rerun()
                else:
                    st.error("Не вдалося зберегти (Sheets / Supabase)")
        with c2:
            if st.button("Усі", key="mgr_tabs_all", use_container_width=True):
                save_manager_tab_visibility({k: True for k in TAB_ORDER})
                st.rerun()
        if st.button("За замовчуванням", key="mgr_tabs_reset", use_container_width=True):
            ...
            save_manager_tab_visibility(MANAGER_TAB_DEFAULTS)
            st.rerun()
