...
def _rozetka_up_invoice_dialog():
    ...
    if prefill.get("epicentr_order_id") or prefill.get("prom_order_id") is not None:
        ...
        if (
            ...
            st.warning(
                f"Індекс не визначено з {src} — після створення перевірте вкладку **Укрпошта** "
...
def _rz_orders_list_fragment():
    ...
    kind_counts = delivery_logos.delivery_kind_counts(order_items, source="rozetka")
    ...
    if len(order_items) < total_count:
        st.caption(
            f"**{delivery_logos.active_delivery_filter_label(key='rz_delivery_filter')}** · "
            f"показано **{len(order_items)}** з **{total_count}**"
        )
    if not order_items:
        st.info(
            "Немає замовлень для цієї служби доставки. "
            "Натисніть **Показати всі** або «Оновити список»."
        )
        return
